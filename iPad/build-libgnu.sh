...
# Backup globals.h before distclean (it gets generated by build-host-tools.sh)
GLOBALS_H_BACKUP=""
if [[ -f "src/globals.h" ]]; then
    GLOBALS_H_BACKUP="$(mktemp)"
    cp "src/globals.h" "$GLOBALS_H_BACKUP"
    echo "Backed up globals.h before distclean"
fi
...
# Restore globals.h after distclean
if [[ -n "$GLOBALS_H_BACKUP" ]] && [[ -f "$GLOBALS_H_BACKUP" ]]; then
    mkdir -p src
    cp "$GLOBALS_H_BACKUP" "src/globals.h"
    rm -f "$GLOBALS_H_BACKUP"
    echo "Restored globals.h after distclean"
fi
...
# Regenerate configure script after applying patch
# This is necessary because configure.ac was modified by the patch
echo "Regenerating configure script with iOS support patch..."
./autogen.sh autoconf
...
# Fix HAVE_ENDIAN_H for iOS (iOS SDK doesn't have endian.h)
echo "Fixing HAVE_ENDIAN_H for iOS..."
if [ -f "lib/config.h" ]; then
    sed -i '' 's/^#define HAVE_ENDIAN_H 1$/#undef HAVE_ENDIAN_H/' lib/config.h || true
fi
...
# Ensure endian.h is generated and fixed before building
if [ -f "endian.h" ]; then
    # Fix endian.h directly (config.status would regenerate it with wrong values)
    # Replace '#if 1' (HAVE_ENDIAN_H=1) with '#if 0' to disable #include_next
    sed -i '' 's/^#if 1$/#if 0/' endian.h 2>/dev/null || true
    # Also remove the #include_next line if it exists
    sed -i '' '/# include_next <endian\.h>/d' endian.h 2>/dev/null || true
    # Remove endian.o if it exists (force recompilation with fixed endian.h)
    rm -f endian.o
fi
